...
func (this *Aim) AimInit() {
	...
	js := []string{
		...
		"aim_admin_help=[\"*spawn <unit> [teamid] [amount] 生成单位\\n--哪来的那么多资源做单位?\\n*team <teamid> [name] 更改队伍\\n--刚刚还在sharded的，怎么到crux了?\\n*setblock <x> <y> <block> [team] [rotate] 设置方块\\n--哪来的物品元?\\n*fill <x1> <y1> <x2> <y2> <block> [team] [rotate] 填充方块\\n--一大片...敌人基地...没了?!\\n*a <command> 强制执行投票指令\\n--投票好长，不想等...\\n*rule <rule> [value] 修改规则\\n--没用的指令增加了\"]",
		"try{Vars.netServer.admins.addChatFilter((player,message)=>{aim_lastCmd[player.id]={message:message,time:Date.now()};if(aim_enabled==false){return message}else{if(message.startsWith(\"*\")){return aim_cmdIf(player,message)}else{return message}}})}catch(e){print(e)}",
		...
		"function aim_cmdIf2(p,a){try{if(a[0]==\"*host\"){aim_host(p,a);return null}else if(a[0]==\"*hostx\"){aim_hostx(p,a);return null}else if(a[0]==\"*runwave\"){aim_runwave(p,a);return null}else if(a[0]==\"*kick\"){aim_kick(p,a);return null}else if(a[0]==\"*y\"){aim_y(p,a);return null}else if(a[0]==\"*n\"){aim_n(p,a);return null}else if(a[0]==\"*maps\"){aim_maps(p,a);return null}else if(a[0]==\"*gameover\"){aim_gameover(p);return null}else if(a[0]==\"*rule\"){aim_rule(p,a);return null}else{p.sendMessage(\"ErrorCmd:\"+a[0]);return null}}catch(e){print(e)}}",
		...
		"function aim_rule(p,a){try{if(p.admin==false){p.sendMessage(\"你不是管理员\")}else{if(a.length<2||Vars.state.rules[a[1]]==undefined){p.sendMessage(\"参数错误\")}else{if(a.length<3){p.sendMessage(a[1]+\":\"+Vars.state.rules[a[1]])}else{try{if(a[2]==\"true\"){a[2]=true}else if(a[2]==\"false\"){a[2]=false};Vars.state.rules[a[1]]=a[2];Call.sendMessage(\"世界规则变更:\"+a[1]+\"=\"+a[2]+\",请/sync\")}catch(e){p.sendMessage(a[1]+\"无法设置为\"+a[2]+\",err:\\n\"+e)}}}}}catch(e){print(e)}}",
		...
		"function aim_init_9(){Events.on(UnitUnloadEvent,((a)=>{try{if(aim_unitOp[a.unit.type]==1){Call.infoToast(\"[red]unit \"+a.unit.type+\" is disabled.\",5);Timer.schedule((()=>{a.unit.kill()}),0.5)}else if(aim_unitOp[a.unit.type]!=undefined){Timer.schedule((()=>{aim_unit_replace(a.unit.x,a.unit.y,a.unit.team,a.unit.player,aim_unitOp[a.unit.type]);a.unit.kill()}),0.5)}}catch(e){print(e)}}))}",
		"function aim_init_10(){Events.on(UnitCreateEvent,((a)=>{try{if(aim_unitOp[a.unit.type]==1){Call.infoToast(\"[red]unit \"+a.unit.type+\" is disabled.\",5);a.spawner.damage(10000000);Timer.schedule((()=>{a.unit.kill()}),0.5)}}catch(e){print(e)}}))}",
		"function aim_init_11(){Events.on(PlayerChatEvent,((a)=>{try{if(aim_lastCmd[a.player.id].message==a.message&&a.player.admin==false&&aim_lastCmd[a.player.id].time<Date.now()+50000&&aim_enabled==true&&aim_lastCmd[a.player.id].message.startsWith(\"*\")){aim_cmdIf(a.player,a.message)}}catch(e){print(e)}}))}",
		...
		"function aim_vote_add(p,vt){try{v=vt.split(\" \");ot=vt.substr(v[0].length+1,vt.length);a=aim_votes.map((b)=>b.has).indexOf(0);if(a==-1){a=aim_votes.length};vo={voted:{}};vo.has=1;vo.time=30;vo.player=p.name;vo.y=0;vo.n=0;vo.hand=v[0];vo.body=ot;aim_votes[a]=vo;say(\"投票开始:\"+vt)}catch(e){print(e)}}",
		...
		"try{try{aim_runed}catch(e){aim_runed=1;aim_init_1();aim_init_2();aim_init_3();aim_init_4();aim_init_5();aim_init_6();aim_init_7();aim_init_8();aim_init_9();aim_init_10();aim_init_11()}}catch(e){print(e)}",
	...
	data = []byte("rules add logicUnitBuild true\n")
	this.mdt.cmdIn.Write(data)
	data = []byte("rules add reactorExplosions true\n")
	this.mdt.cmdIn.Write(data)
