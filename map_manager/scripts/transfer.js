$(function () {
	...
	function fillFilesContainer() {
		...
		for (var i = 0; i < files.length; i++) {
			...
			row.append('<div class="column size">' + formatFileSize(fileInfo.size) + '</div>');
	...
	function checkFileSize(files) {
		...
		if (!$.browser.msie) {
			...
			return STRINGS.UNSUPPORTED_BROWSER_TYPE;
		}
		if (filesize == -1) {
			return STRINGS.UPLOAD_FAILED;
		} else if (filesize > maxsize) {
			return STRINGS.EXCEEDS_FILE_SIZE;
		} else {
			return null;
		}
	}
